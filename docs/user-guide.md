...
> API Doc Manager 화면별 상세 사용 방법

## 시작하기

API Doc Manager는 Swagger/OpenAPI 문서의 버전을 관리하고 변경사항을 추적하는 시스템입니다. 이 가이드를 통해 시스템의 주요 기능을 활용하는 방법을 배울 수 있습니다.

## 페이지 구성

시스템은 4개의 주요 페이지로 구성됩니다:

| 페이지 | 경로 | 용도 |
|--------|------|------|
| 메인 페이지 | `/` | API URL 목록 관리 및 대시보드 |
| API 상세 페이지 | `/api-detail?id=` | Swagger UI 및 버전 히스토리 확인 |
| 버전 비교 페이지 | `/version-compare?id=` | 두 버전 간 상세 비교 |
| Swagger UI 페이지 | `/api-docs?url=` | 독립적인 Swagger UI 뷰어 |
...
| 그룹 | ✅ | 카테고리 분류 (소문자) | "backend" |
| 서비스 | ✅ | 서비스명 (소문자) | "user-service" |
| 설명 | ❌ | 상세 설명 (최대 500자) | "사용자 관리 API" |
| 담당자 | ❌ | 이메일 주소 (슬랙 알림용) | "developer@example.com" |
| 태그 | ❌ | 태그 배열 | ["v2", "production"] |
| 우선순위 | ❌ | low/medium/high | "high" |

3. **"저장"** 버튼 클릭
4. 성공 시:
   - 목록에 새 카드 추가됨
   - 자동으로 Swagger JSON 가져오기 시도
   - 상태 배지로 결과 확인 가능

**유의사항:**
- URL은 반드시 `http://` 또는 `https://`로 시작
- Swagger/OpenAPI JSON 형식이어야 함
- 그룹과 서비스는 자동으로 소문자로 변환됨
- URL 끝의 `/`는 자동으로 제거됨
...
| 버튼 | 기능 | 위치 |
|------|------|------|
| **지금 확인** | Swagger JSON을 다시 가져와서 변경사항 확인 | 상단 헤더 |
| **버전 비교** | 버전 비교 페이지로 이동 | 상단 헤더 |
| **JSON 다운로드** | 현재 버전의 Swagger JSON 파일 다운로드 | 상단 헤더 |
| **삭제** | API 및 모든 버전 삭제 | 상단 헤더 |

**버전 비교 버튼:**
- 클릭 시 `/version-compare?id={API_ID}`로 이동
- 최신 2개 버전이 자동으로 선택됨
...
3. **"확인"** 클릭 시 삭제
4. 삭제 후 메인 페이지로 자동 이동

**주의:**
- 이 작업은 되돌릴 수 없습니다
- 삭제 시 해당 API의 모든 버전 히스토리도 함께 삭제됩니다
- 현재는 연관된 버전 삭제가 미구현 상태입니다 (향후 구현 예정)
...
> 독립적인 Swagger UI 뷰어 페이지입니다.

### 4.1 접근 방법

**URL 파라미터:**
```
/api-docs?url={SWAGGER_JSON_URL}
```

**예시:**
```
/api-docs?url=https://api.example.com/swagger.json
```

### 4.2 기능

- 외부 Swagger JSON URL을 직접 렌더링
- 다크 모드 지원
- 표준 Swagger UI 기능 사용
...
**Q: Swagger JSON을 가져올 수 없습니다.**
- A: URL이 올바른지 확인하세요. `http://` 또는 `https://`로 시작해야 합니다.
- A: 서버에서 인증이 필요할 수 있습니다 (401 에러).
- A: Vercel Password Protection이 활성화되어 있을 수 있습니다.

**Q: 버전이 생성되지 않습니다.**
- A: 변경사항이 없으면 새 버전이 생성되지 않습니다. 기존 버전이 업데이트됩니다.
- A: API가 비활성화되어 있으면 버전을 가져올 수 없습니다.

**Q: 변경사항이 정확하지 않습니다.**
- A: 경로 버전 변경(`/v1/users` → `/v2/users`)은 `path_version_changed` 타입으로 표시됩니다.
- A: 동일한 엔드포인트로 인식되므로 삭제/추가가 아닌 버전 변경으로 표시됩니다.

**Q: 슬랙 알림이 오지 않습니다.**
- A: `SLACK_ENABLED` 환경 변수가 `true`로 설정되어 있는지 확인하세요.
- A: 담당자 이메일이 올바른지 확인하세요.
- A: 슬랙 사용자 ID를 찾을 수 없으면 알림이 전송되지 않습니다.
...
- [API 명세](./api-reference.md) - REST API 엔드포인트 상세
- [아키텍처](./architecture.md) - 시스템 구조 이해
- [핵심 기능](./features.md) - 버전 관리 전략 상세
